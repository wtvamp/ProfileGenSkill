...
**The orb scales to its pane.** Under tmux the overlay is pinned over the persona's own pane, and its diameter is 12% of that pane's width (or 30% of its height, whichever is smaller), clamped to `[--avatar-min, --avatar]` (40–104 pt by default); outside tmux the same rule applies to the whole terminal window. A full-width pane in a typical window lands at the old fixed 104 pt, a third-width pane gets roughly 60 pt instead of an orb covering most of its text, and the name's font and the `--margin` inset follow (so a small orb still hugs its corner). Width leads because text runs horizontally, so that's what a too-big orb covers; the height term only bites on short, wide bottom splits. Both platform implementations use the same fractions so they size identically for the same pane.
