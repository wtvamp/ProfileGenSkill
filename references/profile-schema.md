...
| `image` | yes | string (path) | Path to the persona's **currently selected** picture, relative to the repo root — a static PNG, or an animated GIF if one was generated (see `generation.gif_mode`). One file, not both; a GIF replaces the PNG here rather than sitting alongside it. The markdown body's visible `![name](...)` points at the same file. For a SFW-only persona this is simply its picture; for one with both variants it is a copy of `image_sfw` or `image_nsfw`, whichever `display.variant` selects, and `/display-profile nsfw ...` rewrites it (and the body link) on every switch — so any program reading the profile generically, knowing nothing of variants, shows the selected picture. |
| `image_sfw` | no | string (path) or `null` | Path to the persona's **SFW** picture. Written only for a persona that also has `image_nsfw`, so `image` can move between the two without losing either. A two-variant profile written before this key existed gets it added by its first variant switch. |
| `image_nsfw` | no | string (path) or `null` | Path to the persona's optional **NSFW** picture — the same character from the same seed and base prompt, generated a second time with the NSFW clause added. While `display.variant` is `sfw` it appears in the frontmatter only. Switching to `nsfw` makes it `image` and the body's visible picture too — including inside a tracked CLAUDE.md for a `claude-md` (fully embedded) persona, so keep NSFW personas on `claude-md-ref` + gitignored. |
...
| `variant` | yes | `"sfw"` \| `"nsfw"` | *Which* of the persona's two pictures is currently on screen. `"sfw"` by default, and forced to `"sfw"` at write time for a persona with no `image_nsfw`. Flipped by `/display-profile nsfw on\|off\|toggle` (`toggle_display.py --variant`), which rewrites this line plus `image:` and the body's picture link to match (adding `image_sfw` the first time, for an older profile), and nothing else. `show_profile.py --variant` overrides it for one invocation without changing it. |
