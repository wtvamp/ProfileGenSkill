...
- `scripts/profilegen/` — the reusable package:
  ...
  - `termstate.py` — **the default display path**: iTerm2 badge (persona name) + background image (persona picture). These are terminal *state* rendered outside the text grid, so unlike inline images they survive tmux and every redraw. Also resolves the active tmux pane's tty device so these sequences still reach the user's screen when invoked from a context whose stdout is captured rather than attached to a terminal (an agent's shell tool). See `references/terminal-display.md` for the measured behavior behind this design.
  - `termimg.py` — inline-image display (iTerm2/WezTerm, Kitty/Ghostty, sixel), painted into the text grid at ~10% of terminal width. Used by `--mode inline`; degrades to a clipped sliver under tmux, which is why `termstate.py` is the default instead.
...
- `tests/` — pytest suite (17 files) covering prompt/NSFW logic, ComfyUI node resolution and inventory parsing, template rendering + frontmatter round-tripping, terminal-image protocol detection, iTerm2 badge/background state + pane-tty resolution, display-flag toggling, storage path policy, the mock-backend end-to-end pipeline, and Grok/Grok-CLI backend behavior.
