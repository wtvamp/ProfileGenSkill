...
- `scripts/profilegen/` — the reusable package:
  ...
  - `discovery.py` — shared "find every persona referenced from this project's CLAUDE.md" logic used by `show_profile.py`, `toggle_display.py` and `session_intro.py`. Walks up from the session directory to the nearest CLAUDE.md that actually declares a persona, so subdirectories inherit their project's persona unless a closer CLAUDE.md names its own.
...
- CLI entry points, each printing one JSON object to stdout: `scripts/check_config.py`, `scripts/inspect_comfyui.py`, `scripts/generate_image.py`, `scripts/make_gif.py`, `scripts/write_profile.py`, `scripts/toggle_display.py`, `scripts/cwd_changed.py` (a `CwdChanged` hook that swaps the badge when the session moves into a different persona's directory tree; prints nothing), `scripts/show_profile.py` (the last one is the exception — it writes terminal escape sequences to the user's terminal, and only prints its JSON report when stdout isn't a tty, i.e. when a program rather than a human is reading it).
- `commands/display-profile.md` — the `/display-profile` user command wrapping `show_profile.py`/`toggle_display.py` for interactive on/off control and previewing. Claude Code only discovers commands in a config directory's `commands/`, so it is installed by symlinking this file there (see README → Install); edit it here, never the installed copy.
