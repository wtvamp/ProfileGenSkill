...
- `scripts/profilegen/` — the reusable package:
  ...
  - `hud.py` + `scripts/hud/PersonaHUD.swift` — **the default display path**: a small always-on-top, click-through overlay window (Swift/AppKit `NSPanel`) pinned to a corner of the terminal window, showing the persona's picture (animated, for a GIF) and name. It consumes no terminal rows, touches no user configuration, and depends on no terminal image protocol, so tmux is irrelevant to it. Compiled on first use into `~/.cache/profile-gen`; pid files are per tmux pane so personas in different sessions stay independent, and the overlay asks tmux whether its own window is on screen before showing.
